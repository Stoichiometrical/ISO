...
export function CustomerSegmentRadarChart() {
    ...
    const apiName = getCookie();
    const projectName = getProjectName('project_name');

    useEffect(() => {
        async function fetchData() {
            try {
                const response = await fetch(`${API_1}/${apiName}/get-segment-avg`, {
                    method: 'POST',
                    headers: {
                ...
                // Transform data to the format expected by RadarChart
                const metrics = [
                    'Average_Tenure',
                    'Average_CLV',
                    'Average_Estimated_Profit',
                    'Average_Probability_Alive',
                    'Average_Frequency'
                ];
                ...
                const formattedData = metrics.map(metric => {
                    ...
                    data.forEach(segment => {
                        metricData[segment.Segment] = segment[metric];
                    });
                    return metricData;
                });
        ...
        <div className="flex-grow rounded-xl p-3">
            ...
            <ResponsiveContainer width="100%" height={300}>
                <RadarChart data={radarData}>
                    ...
                    <Tooltip />
                    <Radar name="High Value" dataKey="High Value" stroke="#8884d8" fill="#8884d8" fillOpacity={0.3} />
